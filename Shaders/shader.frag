...
float CalcShadowFactor(vec4 DirectionalLightSpacePos)
{
	...
	// Texture function to get closest depth measured during Shadow Map Pass
	float closestDepth = texture(directionalShadowMap, projCoords.xy).r;
	// Grabing z value form projCords
	float currentDepth = projCoords.z;
	
	vec3 normal = normalize(Normal);
	vec3 lightDir = normalize(directionalLight.direction);
	// Bias used to remove shadow acne
	...
	for(int x = -1; x <= 1; ++x)
	{
		for(int y = -1; y <= 1; ++y)
		{
			// Getting depth values of surrounding texels, PCF - Percentage-Closer Filtering - used to fade shadow edges
			float pcfDepth = texture(directionalShadowMap, projCoords.xy + vec2(x,y) * texelSize).r;
			shadow += currentDepth - bias > pcfDepth ? 1.0 : 0.0;
		}
	}
	
	// Dividing shadow value by number of samples taken
	shadow /= 9.0;
	if(projCoords.z > 1.0)
	{
		shadow = 0.0;
